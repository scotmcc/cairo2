...
// than the prior row's live values.
func TestStateToday_ForwardCopyFromPostDream(t *testing.T) {
	...
	// Today() triggers ensureTodayRow internally, which forward-copies post_dream_*.
	s, err := database.State.Today()
	if err != nil {
		t.Fatalf("Today: %v", err)
	}
...
func TestStateApply_ClampHigh(t *testing.T) {
	...
	if err := database.State.Apply(identity.StateVarConfidence, 5.0); err != nil {
...
func TestStateApply_UpdateCount(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		if err := database.State.Apply(identity.StateVarWarmth, 0.01); err != nil {
