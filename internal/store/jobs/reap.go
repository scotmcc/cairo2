...
// ReapOrphanedTasks sweeps tasks whose subprocess has died, marking them
// failed and triggering job rollup. Takes the raw *sql.DB plus the TaskQ
// and JobQ it needs to drive. Returns the number of tasks reaped.
func ReapOrphanedTasks(sqldb *sql.DB, tasksQ *TaskQ, jobsQ *JobQ) (int, error) {
	...
	for _, r := range stale {
		...
		if task, gerr := tasksQ.Get(r.id); gerr == nil {
			if rerr := jobsQ.ResolveAndUpdateJobStatus(task.JobID); rerr != nil {
