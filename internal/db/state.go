...
func (q *StateQ) ensureTodayRow(today string) error {
	...
	if prior.DreamProcessedAt == nil {
		// q.db is *sql.DB; wrap it in the full *DB so RunDreamRitual can use
		// the StateQ helpers. We build a minimal wrapper referencing q.db.
		wrappedDB := &DB{State: q}
		if _, err := RunDreamRitual(wrappedDB); err != nil {
