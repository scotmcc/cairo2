...
func CurateMemories(database *DB, dreamID int64, threshold float64) error {
	memories, err := database.Memories.UnreviewedWithEmbeddings()
	if err != nil {
	...
	for _, pair := range candidates {
		...
		case MergeOutcomeSkip:
			...
			if logErr := logMemoryConflict(database, dreamID, pair.A, pair.B); logErr != nil {
				fmt.Fprintf(os.Stderr, "curator: log conflict: %v\n", logErr)
			}
		case MergeOutcomeArchive:
			if err := mergeMemory(database, dreamID, winner, loser); err != nil {
...
// both winner and loser. The curator does NOT call MarkReviewed — that's the
// dream-pass closing step.
func CurateFacts(database *DB, dreamID int64, threshold float64) error {
	facts, err := database.Facts.UnreviewedWithEmbeddings()
	if err != nil {
	...
	for _, pair := range candidates {
		...
		if err := mergeFact(database, dreamID, winner, loser); err != nil {
...
func mergeMemory(database *DB, dreamID int64, winner, loser *Memory) error {
	now := time.Now()
...
func mergeFact(database *DB, dreamID int64, winner, loser *Fact) error {
	now := time.Now()
	nowStr := now.Format("2006-01-02 15:04:05")
...
func logMemoryConflict(database *DB, dreamID int64, a, b *Memory) error {
