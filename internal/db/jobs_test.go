...
func Test_Jobs_V030Fields(t *testing.T) {
	...
	j, err := db.Jobs.Create("test job", "description", RoleOrchestrator, nil)
	if err != nil {
...
func Test_Jobs_MigrationPreservesData(t *testing.T) {
	...
	for _, title := range titles {
		if _, err := db.Jobs.Create(title, "desc", RoleOrchestrator, nil); err != nil {
...
func Test_Jobs_ListJobsByStatus(t *testing.T) {
	...
	j1, _ := db.Jobs.Create("pending job", "", RoleOrchestrator, nil)
	j2, _ := db.Jobs.Create("running job", "", RoleOrchestrator, nil)
	j3, _ := db.Jobs.Create("done job", "", RoleOrchestrator, nil)
	_ = db.Jobs.SetStatus(j2.ID, StatusRunning)
	_ = db.Jobs.SetStatus(j3.ID, StatusDone)

	results, err := db.Jobs.ListJobsByStatus([]string{StatusPending, StatusRunning})
	if err != nil {
...
func Test_Jobs_ListJobsWithLiveWorktrees(t *testing.T) {
	...
	_, _ = db.Jobs.Create("old style job", "", RoleOrchestrator, nil)
	...
	j2, _ := db.Jobs.Create("v0.3.0 job", "", RoleOrchestrator, nil)
...
func Test_Jobs_SetReviewed(t *testing.T) {
	...
	j, _ := db.Jobs.Create("review test", "", RoleOrchestrator, nil)
	if err := db.Jobs.SetReviewed(j.ID, StatusAwaitingReview); err != nil {
	...
	if j2.Status != StatusAwaitingReview {
		t.Errorf("status: got %q, want %q", j2.Status, StatusAwaitingReview)
	}
...
func Test_Jobs_ListActiveJobs(t *testing.T) {
	...
	jAwaiting, _ := db.Jobs.Create("awaiting job", "", RoleOrchestrator, nil)
	_ = db.Jobs.SetV030Fields(jAwaiting.ID, &wt.ID, nil, nil, nil, nil, nil, nil)
	_ = db.Jobs.SetStatus(jAwaiting.ID, StatusAwaitingReview)

	jRunning, _ := db.Jobs.Create("running job", "", RoleOrchestrator, nil)
	_ = db.Jobs.SetV030Fields(jRunning.ID, &wt.ID, nil, nil, nil, nil, nil, nil)
	_ = db.Jobs.SetStatus(jRunning.ID, StatusRunning)

	jMerged, _ := db.Jobs.Create("merged job", "", RoleOrchestrator, nil)
	_ = db.Jobs.SetV030Fields(jMerged.ID, &wt.ID, nil, nil, nil, nil, nil, nil)
	_ = db.Jobs.SetStatus(jMerged.ID, StatusMerged)

	// A legacy job (no worktree_id) — must not appear.
	_, _ = db.Jobs.Create("legacy job", "", RoleOrchestrator, nil)
...
func Test_ValidJobStatus(t *testing.T) {
	valid := []string{
		StatusPending, StatusRunning, StatusDone, StatusFailed,
