...
type BridgePayload =
  ...
  | { action: 'list_sessions'; dbPath: string; currentSessionId: number }
  | { action: 'rename_session'; dbPath: string; id: number; name: string; currentSessionId: number }
  | { action: 'delete_session'; dbPath: string; id: number; currentSessionId: number }
...
def list_sessions(conn, db_path, current_session_id):
    sessions = []
    if table_exists(conn, "sessions"):
        ...
        for row in rows:
            ...
            sessions.append({
                ...
                "current": sid == int(current_session_id or 0),
            })
    ...
    try:
        ...
        try:
            ...
            if action == "list_sessions":
                print(json.dumps(list_sessions(conn, db_path, int(payload.get("currentSessionId") or 0))))
                return
            ...
            with conn:
                if action == "rename_session":
                    ...
                    current = int(payload.get("currentSessionId") or 0)
                    if sid == current:
                        die("The active session cannot be deleted while it is open.")
...
export async function readCairoDbSnapshot(dbPath: string): Promise<CairoDbSnapshot> {
  return runBridge<CairoDbSnapshot>({ action: 'snapshot', dbPath });
}

export async function listCairoSessions(
  dbPath: string,
  currentSessionId: number
): Promise<CairoSessionsResult> {
  return runBridge<CairoSessionsResult>({
...
export async function renameCairoSession(
  dbPath: string,
  id: number,
  name: string,
  currentSessionId: number
...
export async function deleteCairoSession(
  dbPath: string,
  id: number,
  currentSessionId: number
): Promise<CairoSessionsResult> {
...
export async function setCairoConfigValue(
  dbPath: string,
  key: string,
  value: string
...
export async function setCairoRoleValue(
  dbPath: string,
  name: string,
  field: 'model' | 'think',
  value: string
...
export async function upsertCairoConsiderAspect(
  dbPath: string,
  name: string,
  traits: string,
  enabled: boolean
): Promise<CairoDbSnapshot> {
  return runBridge<CairoDbSnapshot>({
    action: 'upsert_aspect',
    dbPath,
    name,
    traits,
    enabled,
...
export async function deleteCairoConsiderAspect(
  dbPath: string,
  name: string
): Promise<CairoDbSnapshot> {
...
export async function setCairoConsiderAspectEnabled(
  dbPath: string,
  name: string,
  enabled: boolean
): Promise<CairoDbSnapshot> {
  return runBridge<CairoDbSnapshot>({
    action: 'set_aspect_enabled',
    dbPath,
    name,
    enabled,
  });
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      try {
        ...
        reject(new Error(`Could not parse Cairo database bridge output: ${err?.message || err}`));
      }
    });
